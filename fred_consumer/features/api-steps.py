...
def given_i_have_created_a_facility(step):
  ...
  connector.write(credentials['url'] + "/facilities", {
    'name'       : world.name,
    'active'     : True,
    'coordinates': [0,0]})
...
def then_it_should_appear_at_the_top_of_the_feed (step):
    facilities = connector.get_all_facilities()['facilities']
...
def then_i_should_be_able_to_read_it (step):
    facility = connector.get_json(".json", world.facility['href'])
...
def and_i_should_be_able_to_update_it (step):
    ...
    connector.write(world.facility['href'], world.facility, "PUT")
    facility = connector.get_json(".json", world.facility['href'])
    assert world.facility['name'] == facility['name']
...
def given_i_try_to_create_an_invalid_facility(step):
  try:
    connector.write(credentials['url'] + "/facilities", {'name':"i"})
